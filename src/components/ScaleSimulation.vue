...
import { SCALE_BEAM_LENGTH, SCALE_PAN_WIDTH, ALLOWED_ERROR } from '@/constants'

const scaleStore = useScaleStore()
const canvasContainer = ref<HTMLDivElement | null>(null)

const leftWeight = computed(() => scaleStore.leftWeight)
const rightWeight = computed(() => scaleStore.rightWeight)
...
function updateScaleBalance() {
  if (!beam || !engine) return

  const diff = leftWeight.value - rightWeight.value
  const maxAngle = 0.5

  let targetAngle = 0
  if (Math.abs(diff) > ALLOWED_ERROR) {
...
function updateWeights() {
  ...
  const leftPlaced = scaleStore.placedWeights.filter(w => w.placedOn === 'left')
  const rightHerbCount = scaleStore.herbCount
  const herbUnitWeight = scaleStore.currentHerb.unitWeight

  leftPlaced.forEach((pw, index) => {
    const weightSize = Math.min(30 + pw.weight.weight * 2, 50)
  ...
  const totalHerbWeight = rightHerbCount * herbUnitWeight
  ...
  for (let i = 0; i < herbStackCount; i++) {
    ...
    const herbBody = Matter.Bodies.circle(
      ...
      {
        render: {
          fillStyle: scaleStore.currentHerb.color,
          strokeStyle: '#654321',
          lineWidth: 1
        },
        density: 0.0005
...
watch(
  () => [scaleStore.placedWeights.length, scaleStore.herbCount],
  () => {
    updateWeights()
  },
  { deep: true }
