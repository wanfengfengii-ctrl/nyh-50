...
const placedWeights = computed(() => clinicStore.getPlacedWeights())
...
function isWeightPlaced(id: string): boolean {
  return clinicStore.placedWeightIds.includes(id)
}

function isWeightUsed(id: string): boolean {
  return clinicStore.isWeightUsed(id)
}
...
function handleWeightClick(weight: Weight) {
  ...
  if (isWeightPlaced(weight.id)) {
    clinicStore.removeWeight(weight.id)
  } else {
    clinicStore.addWeight(weight)
  }
}

function handleRemoveWeight(id: string) {
  clinicStore.removeWeight(id)
}
</script>
